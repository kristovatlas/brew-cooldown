...
1. **Lifetime-floor gate (`M`, default 1 day) defends against revert-within-hours.** A malicious version introduced and reverted within hours has total lifetime < `M` and fails the gate; we don't install from its introduction commit even once the `N`-day cooldown elapses. See ADR-0010 Scenario B.
2. **`unknown:*` synthetic versions are conservatively skipped.** A commit whose message we couldn't parse is bucketed under a unique `unknown:<sha>` synthetic version and the picker refuses to install from any such version, even if its own age and HEAD-time would individually clear both gates. Defends against hand-crafted commits with non-standard messages.
3. **Cooldown gate (`N`) still defends against fresh in-lifetime attacker commits.** Even though the refined picker (per ADR-0010's Revision) installs from the latest in-V recognized commit rather than the first-introduction commit, that commit must be ≥ `N` days old. An attacker who lands a commit in V's lifetime today won't be picked for at least `N` days — by which time, if the attack is detectable, `homebrew-core`'s PR review or downstream security scanning should have caught it.
...
2. **In-lifetime spoofed-rebuild attacks that age past the cooldown.** This is ADR-0010 Scenario C, weakened by the Revision: an attacker who lands a commit with a spoofed `<formula>: update X.Y.Z bottle.` message within a legitimate version's lifetime, and whose commit then ages past `N` days unreverted, **can be selected as the install target** under the refined picker (which picks the latest in-V recognized commit ≥ `N` days old, not the first-introduction commit). The original ADR-0010 rule pinned to first-introduction specifically to defend against this; the Revision traded that defense for substantially better bottle availability on real-world fast-mover formulae. Defense against this case now relies on `homebrew-core`'s **PR review process** catching the malicious diff before it ages into the cooldown window. Users for whom Scenario C matters specifically can `--strict-cooldown` to fall back to ADR-0008 N-stable (a different but also commit-immutable picker).
3. **Dependence on BrewTestBot commit-message format.** The version-introduction rule parses commit messages (`<formula> X.Y.Z`, `<formula>: update X.Y.Z bottle.`) to extract version labels. If Homebrew changes its conventions or a maintainer commits with a non-standard message, our parser conservatively buckets the affected commit as `unknown:<sha>` (each such commit becomes its own synthetic version and never accumulates enough lifetime to qualify, and the picker explicitly skips all `unknown:*` versions). **This is fail-closed** — we hold formulae we'd otherwise rewind rather than installing the wrong thing — but it means our default behavior depends on Homebrew's upstream conventions remaining stable. `--strict-cooldown` is the escape hatch when that bites.
...
10. **Bottle availability changes outside our control.** Both rewind rules pick a historical formula commit and ask brew to install via `--force-bottle`. If the bottle blob for the chosen commit has been GC'd from `ghcr.io` / Homebrew's CDN, brew refuses with `CannotInstallFormulaError` and the user sees a held verdict. **This is by design**: we explicitly choose unavailability over a source-build, because the historical formula's `install` Ruby block is itself attacker-influenceable — the very thing the cooldown is defending against. The ADR-0010 *Revision* picker (latest in-V recognized commit ≥ N days old) deliberately avoids the original-bottle-SHA problem by preferring the latest rebuild within V's lifetime; bottle-unavailability still happens for very old picks or for formulae where ghcr.io GC reached even the latest in-V rebuild. Users who hit this can `--no-rewind` (held with eligibility date for the latest), `--no-cooldown` (if independently verified), or use `brew install` directly.
